...
# ckpt directory.
#
# These MUST match the active training config in ``train/run.sh``; otherwise
# once the fallback path is actually taken the built model may shape-mismatch
...
_FALLBACK_MODEL_CFG = {
    ...
    'use_time_context_encoder': False,
    'time_context_hidden_mult': 2,
    'time_context_dropout': 0.0,
    'query_generator_type': 'vanilla',
    'temporal_query_alpha': 0.1,
    'temporal_query_use_base_query': True,
    'use_token_se': False,
    'token_se_gamma_init': 0.01,
    'token_se_hidden_mult': 0.25,
    'token_se_dropout': 0.0,
    ...
    'dense_stat_clamp_value': 18000000.0,
    'use_dense_group_projector': True,
    'use_quantile_trend_encoder': True,
    'dense_group_projector_version': 'v7',
    'use_time_context_features': True,
    'time_context_emb_dim': 16,
    'hash_bucket_size': 100000,
    'use_item_query_adapter': True,
    'item_query_gate_init': -4.0,
    'use_recency_query_adapter': True,
    'recency_query_gate_init': -4.0,
    'recency_query_dropout': 0.0,
    'use_stat_pair_residual': True,
    'stat_pair_gate_init': -4.0,
    'use_ns_cross_adapter': True,
    'ns_cross_low_rank': 64,
    'ns_cross_dropout': 0.05,
    'ns_cross_gate_init': -4.0,
    'use_seq_time_film': False,
    'seq_time_film_gate_init': -2.5,
    'seq_time_dropout': 0.02,
    'use_target_time_din': False,
    'din_hidden_mult': 3,
    'din_dropout': 0.02,
    'din_query_gate_init': -2.5,
    'din_output_gate_init': -2.8,
    'din_history_dropout': 0.08,
    'use_dcn_ns_cross': False,
    'dcn_ns_cross_layers': 2,
    'dcn_ns_cross_low_rank': 64,
    'dcn_ns_cross_dropout': 0.05,
    'use_ns_output_fusion_residual': False,
    'ns_output_fusion_gate_init': -3.2,
    'ns_output_fusion_dropout': 0.02,
    'use_strong_time_residual': False,
    'time_residual_gamma_init': 0.03,
    'use_gamma_writeback': False,
    'gamma_q_init': 0.05,
    'gamma_ns_init': 0.03,
}
...
def build_model(
    ...
    user_int_feature_fids = [
        fid for fid, _, _ in dataset.user_int_schema.entries
    ]
...
def _batch_to_model_input(
    ...
    seq_time_calendars: Dict[str, torch.Tensor] = {}
    for domain in seq_domains:
        ...
        seq_time_deltas[domain] = device_batch.get(
            ...
            torch.zeros(B, L, dtype=torch.float, device=device))
        seq_time_gaps[domain] = device_batch.get(
            f'{domain}_time_gap',
            torch.zeros(B, L, dtype=torch.float, device=device))
        seq_time_calendars[domain] = device_batch.get(
            f'{domain}_time_calendar',
            torch.zeros(B, L, 4, dtype=torch.float, device=device))
    ...
    return ModelInput(
        ...
        time_context_feats=device_batch.get(
            'time_context_feats',
            torch.zeros(
                device_batch['user_dense_feats'].shape[0],
                2,
                dtype=torch.long,
                device=device,
            ),
        ...
        seq_time_calendars=seq_time_calendars,
    )

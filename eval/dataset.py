...
class PCVRParquetDataset(IterableDataset):
    ...
    def __init__(
        ...
        for domain in self.seq_domains:
            ...
            self._buf_seq_cal[domain] = np.zeros((B, max_len, 4), dtype=np.float32)
    ...
    def _convert_batch(self, batch: "pa.RecordBatch") -> Dict[str, Any]:
        ...
        beijing_timestamps = timestamps + 8 * 3600
        beijing_days = np.floor_divide(beijing_timestamps, 86400)
        beijing_hour = np.floor_divide(np.mod(beijing_timestamps, 86400), 3600)
        beijing_weekday = np.mod(beijing_days + 3, 7)
        time_context_feats = np.stack(
            [beijing_hour, beijing_weekday], axis=1).astype(np.int64)
        if self.is_training:
        ...
        result = {
            ...
            'time_context_feats': torch.from_numpy(time_context_feats),
            'user_id': user_ids,
        ...
        for domain in self.seq_domains:
            ...
            if ts_ci is not None:
                ...
                for i in range(B):
                    ...
                    if l > 0 and l < max_len:
                        gaps[i, l - 1] = 0.0
                time_gap[:] = np.log1p(gaps / 60.0)

                bj_ts = ts_padded + 8 * 3600
                hour = np.floor_divide(np.mod(bj_ts, 86400), 3600).astype(np.float32)
                day = np.floor_divide(bj_ts, 86400)
                weekday = np.mod(day + 3, 7).astype(np.float32)
                cal = np.stack([
            ...
            result[f'{domain}_time_calendar'] = torch.from_numpy(time_calendar.copy())
...
def get_pcvr_data(
    ...
    **kwargs: Any,
    ...
    actual_valid_rgs = total_rgs - n_train_rgs
    train_row_ratio = train_rows / max(1, total_rows)
    valid_row_ratio = valid_rows / max(1, total_rows)

    logging.info(
        f"Row Group split: total_rgs={total_rgs}, requested_valid_ratio={valid_ratio}, "
        f"train_rgs={n_train_rgs}, valid_rgs={actual_valid_rgs}")
    logging.info(
        f"Sample split: total_rows={total_rows}, train_rows={train_rows} "
        f"({train_row_ratio:.4%}), valid_rows={valid_rows} ({valid_row_ratio:.4%})")

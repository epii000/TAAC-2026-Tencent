...
class ModelEMA:
    """Small EMA helper for validation and checkpoint smoothing."""
    ...
    def resync(self, model: nn.Module, reinit_ptrs: set) -> None:
        for name, p in model.named_parameters():
            if name in self.shadow and p.data_ptr() in reinit_ptrs:
...
class PCVRHyFormerRankingTrainer:
    ...
    def __init__(
        ...
        if lr_schedule == 'cosine':
            est_total_steps = max(1, num_epochs * len(train_loader))
            def _lr_lambda(step: int) -> float:
                if step < self.warmup_steps:
                    return step / max(1, self.warmup_steps)
                progress = (step - self.warmup_steps) / max(1, est_total_steps - self.warmup_steps)
        ...
        logging.info(f"PCVRHyFormerRankingTrainer loss_type={loss_type}, "
                     ...
                     f"weight_decay={self.weight_decay}, lr_schedule={lr_schedule}, "
                     f"ema_decay={ema_decay}, label_smoothing={self.label_smoothing}, "
    ...
    def _make_model_input(self, device_batch: Dict[str, Any]) -> ModelInput:
        ...
        for domain in seq_domains:
            ...
            seq_time_calendars[domain] = device_batch.get(
                ...
                torch.zeros(B, L, 4, dtype=torch.float, device=self.device))
        return ModelInput(
    ...
    def evaluate(self, epoch: Optional[int] = None) -> Tuple[float, float]:
        ...
        logging.info("Validation phase start: eval_loop dataloader_to_model")
        with torch.inference_mode():
            for step, batch in pbar:
                logits, labels = self._evaluate_step(batch)
                all_logits_list.append(logits.detach().cpu())
                all_labels_list.append(labels.detach().cpu())
        logging.info("Validation phase end: eval_loop dataloader_to_model")

        logging.info("Validation phase start: cpu_concat_and_numpy")
        ...
        logging.info(
            f"Validation phase end: cpu_concat_and_numpy samples={len(all_logits):,}")
        ...
        if len(probs) == 0 or len(np.unique(labels_np)) < 2:
            ...
            logging.info("Validation phase start: sklearn_roc_auc_score")
            auc = float(roc_auc_score(labels_np, probs))
            logging.info("Validation phase end: sklearn_roc_auc_score")

        # Binary logloss (same NaN filtering).
        logging.info("Validation phase start: cpu_logloss")
        ...
        if len(valid_logits) > 0:
            ...
            logloss = float('inf')
        logging.info("Validation phase end: cpu_logloss")
